...
const generatePDF = async () => {
  ...
  const renderContent = async () => {
    ...
    doc.text(`Address: ${buyerAddr || "-"}`, pageWidth / 2 + 5, currentY, { maxWidth: 80 });
    ...
    doc.text(`₹ ${grandTotal.toFixed(2)}`, pageWidth - 20, currentY, { align: "right" });
    ...
    currentY += drawBlock("Bank Details:", bank, startX, currentY, footerWidth);
    currentY += 6;
    currentY += drawBlock("Terms & Conditions:", terms, startX, currentY, footerWidth);
  ...
  const emailInvoice = () => {
    window.location.href = `mailto:?subject=GST Invoice ${invoiceNo}&body=Invoice generated using BillingSoftwareOnline.com`;
  };
