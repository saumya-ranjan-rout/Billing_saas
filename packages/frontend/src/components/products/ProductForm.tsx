...
const productSchema = z.object({
  ...
  hsnCode: z.string()
    ...
    .nonempty("HSN Code is required"),

  // must be > 0
  costPrice: z.number({
    required_error: "Cost price is required"
  }).gt(0, "Cost price must be greater than 0"),
  ...
  // default but conditionally required for goods
  stockQuantity: z.number().gte(0),
  ...
  // ⭐ Conditional validation for GOODS
  .refine(
    data => {
      if (data.type === "goods") {
        return data.stockQuantity > 0; // must be > 0
      }
      return true;
    },
    { message: "Stock quantity must be greater than 0 for goods", path: ["stockQuantity"] }
  )
  .refine(
    data => {
      if (data.type === "goods") {
    ...
    { message: "Unit is required for goods", path: ["unit"] }
  );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
        ...
        <div>
          <Input
            label={<span>Product Name <Required /></span>}
            {...register('name')}
        ...
        {/* Product Type */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Product Type <Required /></label>
          <Select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input
          label={<span>SKU <Required /></span>}
          {...register('sku')}
          error={errors.sku?.message}
          disabled={isSubmitting}
        />
        <Input
          label={<span>HSN Code <Required /></span>}
          {...register('hsnCode')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Input
          label={<span>Cost Price (₹) <Required /></span>}
          type="number"
          step="0.01"
        ...
        />
        <Input
          label={<span>Selling Price (₹) <Required /></span>}
          type="number"
          step="0.01"
      ...
      {productType === 'goods' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Input
            label={<span>Stock Quantity <Required /></span>}
            type="number"
            step="0.01"
      ...
      <div className="space-y-2">
        ...
        {!isCreatingNewCategory ? (
          <div className="space-y-2">
            ...
            <label className="block text-sm font-medium text-gray-700 mb-1">Category <Required /></label>
