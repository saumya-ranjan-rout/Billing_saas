...
const invoiceItemSchema = z.object({
  ...
  taxRate: z.number().min(0, 'Tax cannot be negative').max(100).default(0),

  tax_type: z.string().min(1, 'Tax Type is required').optional(),

  has_cess: z.boolean().default(false),

  cess_value: z.number().min(0, 'CESS cannot be negative').max(100).default(0),
});
...
const invoiceSchema = z.object({
  ...
  paymentMethod: z.enum([
    "cash", "bank_transfer", "cheque", "credit_card", "debit_card", "upi", "wallet", "other"
  ]),
...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice, onSuccess, onCancel }) => {
  ...
  const sameAsShipping = watch("shippingAddress");
...
useEffect(() => {
  if (copyAddress) {
    setValue("billingAddress", sameAsShipping || "");
  } else {
    setValue("billingAddress", "");   // 👈 CLEAR when unchecked
  }
}, [copyAddress, sameAsShipping, setValue]);
  ...
  label={<span>Issue Date <Required /></span>}
  type="date"
  min={new Date().toISOString().split("T")[0]}   // 👈 BLOCK PAST DATE
  {...register('issueDate')}
  error={errors.issueDate?.message}
  disabled={isSubmitting}
/>
